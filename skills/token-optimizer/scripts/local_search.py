...
def get_or_create_chunks(
    conn: sqlite3.Connection, filepath: str, content: str
) -> list[tuple[int, int, str, list[float]]]:
    file_hash = compute_sha256(content)
    cur = conn.cursor()
    cur.execute(
        "SELECT chunk_idx, start_line, end_line, snippet, embedding FROM file_chunks WHERE filepath = ? AND file_sha256 = ?",
        (filepath, file_hash),
    )
    rows = cur.fetchall()
...
def search(query: str, root_dir: str = ".", top_k: int = 5):
    try:
        ...
        print(
            f"[ERROR] No se pudo conectar con Ollama ({EMBED_MODEL}): {e}",
            file=sys.stderr,
        )
        return
